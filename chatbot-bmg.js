...
4. 如果问到价格："我可以帮你联系Alejandro +34 634 268 663"`
  ...
  function initBMGBot() {
    ...
    const existingContainer = document.getElementById('bmg-chat-container');
    const existingButton = document.getElementById('bmg-chat-button');
    const existingStyle = document.querySelector('style[data-bmg-style]');
    ...
    style.setAttribute('data-bmg-style', 'true');
    style.textContent = `
    ...
    // HTML - Usando el logo de BMG
    const chatHTML = `
      <button id="bmg-chat-button">
        <div class="bmg-logo-icon" style="width: 24px; height: 24px;">BMG</div>
        <span>BMG Chat</span>
      </button>
      ...
      <div id="bmg-chat-container">
        <div id="bmg-chat-header">
          <div class="bmg-logo">
            <div class="bmg-logo-icon">BMG</div>
            <div class="bmg-logo-text">
              <div class="bmg-logo-main">BridgeMind</div>
              <div class="bmg-logo-sub">Games Assistant</div>
            </div>
          </div>
    ...
    isInitialized = true;
    console.log('✅ BMG Chatbot inicializado correctamente');
  }

  // Inicializar cuando el DOM esté listo
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initBMGBot);
  } else {
    initBMGBot();
  }
})();
